    ...
    <form action={formAction} className="flex flex-col gap-6">
      <div className="grid gap-6 sm:grid-cols-2">
        <div>
          ...
          <input id="title" name="title" required defaultValue={promotion?.title} className="w-full border border-line px-4 py-2.5" />
        </div>
        <div>
      ...
      <div>
        ...
        <textarea id="description" name="description" required rows={5} defaultValue={promotion?.description ?? ""} className="w-full border border-line px-4 py-2.5" />
      </div>

      <div className="grid gap-6 sm:grid-cols-2">
        <div>
          <label htmlFor="validFrom" className="mb-1.5 block text-sm font-medium text-ink">
            Obowiązuje od (opcjonalnie)
          </label>
          <input id="validFrom" name="validFrom" type="date" defaultValue={promotion?.valid_from ?? ""} className="w-full border border-line px-4 py-2.5" />
        </div>
        <div>
          <label htmlFor="validUntil" className="mb-1.5 block text-sm font-medium text-ink">
            Obowiązuje do (opcjonalnie)
          </label>
          <input id="validUntil" name="validUntil" type="date" defaultValue={promotion?.valid_until ?? ""} className="w-full border border-line px-4 py-2.5" />
        </div>
      </div>
      ...
      <fieldset className="border border-line p-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <input id="seoTitle" name="seoTitle" defaultValue={promotion?.seo_title ?? ""} className="w-full border border-line px-4 py-2.5" />
          </div>
          <div>
            <label htmlFor="seoDescription" className="mb-1.5 block text-sm text-ink-soft">
              SEO description
            </label>
            <input id="seoDescription" name="seoDescription" defaultValue={promotion?.seo_description ?? ""} className="w-full border border-line px-4 py-2.5" />
          </div>
        </div>
      </fieldset>
      ...
      <div>
        <label htmlFor="cover" className="mb-1.5 block text-sm font-medium text-ink">
          Zdjęcie okładkowe {promotion?.cover_image_path && "(wgraj nowe, aby zastąpić obecne)"}
        </label>
        <input
          id="cover"
          name="cover"
      ...
      <label className="flex items-center gap-2 text-sm text-ink">
        ...
        Aktywna (widoczna publicznie pod swoim adresem)
      </label>
      <p className="-mt-4 text-xs text-ink-faint">
        Promocja z minioną datą „Obowiązuje do” automatycznie zniknie z listy aktualnych promocji, ale pozostanie
        dostępna pod swoim adresem jako archiwalna — dzięki temu stare, zaindeksowane linki nie prowadzą do błędu 404.
      </p>
